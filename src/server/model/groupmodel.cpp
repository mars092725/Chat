...
bool GroupModel::createGroup(Group &group)
{
    ...
    if (mysql.connect())
    {
        ...
        // 如果创建相同的群组，此时update将会失败，返回false
    }
...
vector<Group> GroupModel::queryGroups(int userid)
{
    ...
    2. 这里的群组信息指的是群id，群名以及群描述，这些信息都需要返回客户端，这里使用的依旧是多表联合查询，也是数据库查询的优化思路，数据库本身磁盘IO操作比较多，能一次性做完的操作一次性做完。
    3. 在根据群组信息，查询属于该群组的所有用户的userid，并且和user表进行多表联合查询，查出用户的详细信息
    */
    char sql[1024] = {0};
